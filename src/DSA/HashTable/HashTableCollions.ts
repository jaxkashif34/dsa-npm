export class HashTableCollions {
  size: number;
  ...
  set(key: string, value: string) {
    ...
    if (!bucket) {
      // initialize new bucket and set the value
      this.table[index] = [[key, value]];
    } else {
      const sameKeyItem = bucket.find((item) => item[0] === key);
      if (sameKeyItem) sameKeyItem[1] = value;
      // if key found in the bucket update the value of that key in the bucket else add new item into the bucket
      // how bucket looks like [   [   ["name" "Alex"] item  ["age" "32"] item   ] bucket     ]  main array
